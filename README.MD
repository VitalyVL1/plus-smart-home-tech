...
- **config-server** - Сервер централизованной конфигурации на основе Spring Cloud Config. Позволяет микросервисам подключаться к внешним конфигурациям, что упрощает управление настройками в разных средах.
- **discovery-server** - Сервер обнаружения микросервисов на основе Spring Cloud Eureka. Позволяет микросервисам обнаруживать друг друга и взаимодействовать.
...
- **analyzer** - Сервис анализа телеметрии умного дома. Обрабатывает снимки состояния сенсоров, проверяет условия сценариев и отправляет команды на выполнение действий.
- **serialization** - Модуль с Avro и Protobuf схемами для сериализации/десериализации сообщений Kafka и gRPC взаимодействия.
